...
// Description:
//			Fun fun fun maze simulation stuff
...
//cSmartArray_Template<std::string> modelLocations;
...
struct sProcessPlantsDetails
{
	float deltaTime;
	bool bIsDone;
};

struct sProcessAnimalDetails
{
	float deltaTime;
	bool bIsDone;
	cAnimal* ani;
...
DWORD WINAPI ProcessPlants(LPVOID pVoidIDK/*cSmartArray_Template<cPlantEntity*> plants*/);
DWORD WINAPI ProcessAnimal(LPVOID pVoidAnimalDetails);
...
int main(int argv, char** argc)
{
	...
	// Question 1 stuff I guess, it's kinda spread throughout
	// Maze stuff first
	cMazeMaker* mazeMaker = new cMazeMaker();
	mazeMaker->GenerateMaze(99, 99);
	mazeMaker->PrintMaze();
	mazeMaker->PrintMazeToFile("maze.txt");
	maze = mazeMaker->maze;						// make a local so it's a little easier to call
	// this is literally no different than just using mazeMkaer->maze wherever

	cSmartArray_Template<cSmartArray_Template<cSmartArray_Template<bool>>> saMaze;
	for (unsigned int indexA = 0; indexA != maze.size(); indexA++)
	{
		cSmartArray_Template<cSmartArray_Template<bool>> saSABools;	// smart array of smart array of bools here to insert after second for loop
		for (unsigned int indexB = 0; indexB != maze[indexA].size(); indexB++)
		{
			bool tempB = maze[indexA][indexB][0];
			cSmartArray_Template<bool> saBools;	// sa of bools get the bool inserted into
			saBools.insertAtEnd(tempB);
			saSABools.insertAtEnd(saBools);		// then the sa of bools gets inserted into the saSABools
		}
		saMaze.insertAtEnd(saSABools);			// and when we're done the row we insert into the maze
	}
	// [x][y][grass/sand]
	//saMaze.getAtIndex(0).getAtIndex(0).getAtIndex(0);

	// Threading fun fun FUN
	const unsigned int NUMTHREADS = 10;		// update as I figure out how many threads I need, 1 for plants, 1 per animal
	DWORD threadIDs[NUMTHREADS] = { 0 };
	//cSmartArray_Template<DWORD> threadIDs;
	//for (unsigned int index = 0; index != NUMTHREADS; index++)
	//{
	//	threadIDs.insertAtEnd(0);
	//}
	...
	//cShaderManager::cShader fragShader;
	//ss.str("");
	//ss << SOLUTION_DIR << "common\\assets\\shaders\\fragShader_01 - original.glsl";
	//fragShader.fileName = ss.str();
...
//					// 2 = directional light
	...
	// this is how I typically load the models into the world
	...
	// for Algoirthms midterm, new thing of making the cubes in the shape of the maze and colouring them
	// depending on whether they're walls or path

	if (makeMazeModels())
	{
		std::cout << "makeMazeModels finished OK" << std::endl;
	}
	else
	{
		std::cout << "makeMazeModels did not finish OK, aborting" << std::endl;
		return -1;
	}

	//makePlants(maze);
	makePlants(saMaze);
	makeHerbivores(saMaze);
	makeCarnivores(saMaze);
	...
	while (!glfwWindowShouldClose(pWindow))
	{
		...
		for (unsigned int index = 0; index != ::g_vec_pMeshes.size(); index++)
		{
			...
			{
				// for testing
				if (glm::distance(pCurrentMesh->positionXYZ, ::cameraEye) < 1000.0f)
				{
					DrawObject(pCurrentMesh,
						matModel,
						matModel_Location,
		...
		sProcessPlantsDetails plantDetails;
		plantDetails.deltaTime = deltaTime;
		plantDetails.bIsDone = false;
		sProcessPlantsDetails* pPlantDetails = &plantDetails;

		void* pVoidPlantsDetails = (void*) pPlantDetails;// = void*;
		// search details... yeah idk
		HANDLE hProcessPlantsThreads = CreateThread(
			NULL,						// Not going to change thread attributes
			0,							// default stack size 1 Mbyte
			ProcessPlants,				// HAS to be in the LPTHREAD_START_ROUTINE format
			pVoidPlantsDetails,			// parameters to pass, type is LPVOID
			0,							// Start it right aaway 
			&threadIDs[0]				// LPDWORD for "long pointer to DWORD"
		);

		DWORD waitVal = WaitForSingleObject(hProcessPlantsThreads, INFINITE);
		if (waitVal == WAIT_TIMEOUT)
		{
			std::cout << "(main) I waited long enough!\n";
		}

		DWORD ProcessPlantsThreadExitCode = 0;
		GetExitCodeThread(hProcessPlantsThreads, (LPDWORD)(&ProcessPlantsThreadExitCode));
		//std::cout << "Process plants thread returned: " << ProcessPlantsThreadExitCode << std::endl;
		...
		for (unsigned int index = 0; index != ::plants.getSize(); index++)
		{
			...
			DrawObject(((Entity*)::plants.getAtIndex(index))->GetMesh(),
				matModel,
				matModel_Location,
				matModelInverseTranspose_Location,
				program,
		...
		// Question 3
		// Animals
		sProcessAnimalDetails animalDetails[NUMTHREADS];
		HANDLE hProcessAnimalsThreads[NUMTHREADS];
		for (unsigned int index = 0; index != ::animals.getSize(); index++)
		{
			animalDetails[index].ani = ::animals.getAtIndex(index);
			animalDetails[index].bIsDone = false;
			animalDetails[index].deltaTime = deltaTime;

			sProcessAnimalDetails* pAnimalDetails = &(animalDetails[index]);
			void* pVoidAnimalDetails = (void*)pAnimalDetails;

			// will have to redo this and update the delta time per frame
			hProcessAnimalsThreads[index] = CreateThread(
				NULL,
				0,
				ProcessAnimal,
				pVoidAnimalDetails,
		...
		for (unsigned int index = 0; index != ::animals.getSize(); index++)
		{
			ResumeThread(hProcessAnimalsThreads[index]);
		}

		DWORD startTime_ms = GetTickCount();

		//std::cout << "(main waiting for worker threads)\n";

		// This is a "blocking call" meaning this thread is stalled until the worker thread returns
		waitVal = WaitForMultipleObjects(NUMTHREADS - 1,
			hProcessAnimalsThreads,
			FALSE, // Wait for all of them to finish
			INFINITE);
		...
		{
			matModel = glm::mat4(1.0f);
			DrawObject(((Entity*)::animals.getAtIndex(index))->GetMesh(),
				matModel,
				matModel_Location,
				matModelInverseTranspose_Location,
				program,
...
bool makeMazeModels()
{
	...
	for (unsigned int indexA = 0; indexA != maze.size(); indexA++)				
	{
		for (unsigned int indexB = 0; indexB != maze[indexA].size(); indexB++)
		{
			...
			//tempMesh->positionXYZ = glm::vec3(99 - indexB/* * 0.1f*/, 99 - indexA/* * 0.1f*/, 0.0f);
			tempMesh->positionXYZ = glm::vec3(indexA, indexB, 0.0f);
			tempMesh->orientationXYZ = glm::vec3(0.0f, 0.0f, glm::radians(90.0f));
			tempMesh->scale = glm::vec3(0.1f);
			tempMesh->scale = glm::vec3(1.0f);
			tempMesh->bDontLight = true;
			tempMesh->bUseObjectDebugColour = true;
			tempMesh->bUseWholeObjectDiffuseColour = true;
			if (maze[indexA][indexB][0])
			{
				// if true it's a grass, if false it's a sand
				// if true it's a wall, if false it's a path
				tempMesh->wholeObjectDiffuseRGBA = glm::vec4(0.0f, 0.8f, 0.0f, 1.0f);
				tempMesh->objectDebugColourRGBA = glm::vec4(0.0f, 0.8f, 0.0f, 1.0f);
			}
			else
			{
				tempMesh->wholeObjectDiffuseRGBA = glm::vec4(204.0f / 255.0f, 204.0f / 255.0f, 0.0f, 1.0f);
				tempMesh->objectDebugColourRGBA = glm::vec4(204.0f / 255.0f, 204.0f / 255.0f, 0.0f, 1.0f);
			}

			//cMesh* tempChild = new cMesh();
			//tempChild->meshName = "Quad_1_sided_aligned_on_XY_plane.ply";
			//tempChild->positionXYZ = glm::vec3(0.0f, 0.0f, 0.0f);
			//tempChild->orientationXYZ = glm::vec3(0.0f);
			//tempChild->scale = glm::vec3(1.1f);
			//tempChild->bDontLight = true;
			//tempChild->bUseObjectDebugColour = true;
			//tempChild->wholeObjectDiffuseRGBA = glm::vec4(0.0f);
			//tempChild->bUseWholeObjectDiffuseColour = true;
			//tempChild->objectDebugColourRGBA = glm::vec4(0.0f);
			//tempChild->bIsWireframe = true;

			//tempMesh->vec_pChildMeshes.push_back(tempChild);
...
void loadMidtermModels()
{
	...
	modelLocations.push_back("SM_Generic_Tree_01.ply");					// for plants
...
DWORD WINAPI ProcessPlants(LPVOID pVoidIDK/*cSmartArray_Template<cPlantEntity*> plants*/)
{
	sProcessPlantsDetails* pPlantDetails = (sProcessPlantsDetails*)pVoidIDK;
	pPlantDetails->bIsDone = false;
	float deltaTime = pPlantDetails->deltaTime;	// accidentally deleted this, thread works :)

	unsigned int saSize = ::plants.getSize();

	// draw and update the plant entities
	for (unsigned int index = 0; index != saSize; index++)
	{
		::plants.getAtIndex(index)->Process(deltaTime);
	}
	
	pPlantDetails->bIsDone = true;
...
{
	srand(time(NULL));
	sProcessAnimalDetails* pAnimalDetails = (sProcessAnimalDetails*)pVoidAnimalDetails;
	pAnimalDetails->bIsDone = false;
	float deltaTime = pAnimalDetails->deltaTime;
	
	pAnimalDetails->ani->Process(deltaTime);

	pAnimalDetails->bIsDone = true;
	return 1;
}